...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TABLE "variants" (
  	...
  	"price_in_c_o_p_enabled" boolean,
  	"price_in_c_o_p" numeric,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  ...
  CREATE TABLE "_variants_v" (
  	...
  	"version_price_in_c_o_p_enabled" boolean,
  	"version_price_in_c_o_p" numeric,
  	"version_updated_at" timestamp(3) with time zone,
  	"version_created_at" timestamp(3) with time zone,
  ...
  CREATE TABLE "carts" (
  	...
  	"subtotal" numeric,
  	"currency" "enum_carts_currency" DEFAULT 'COP',
  	"note" varchar,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  CREATE TABLE "orders" (
  	...
  	"amount" numeric,
  	"currency" "enum_orders_currency" DEFAULT 'COP',
  ...
  CREATE TABLE "transactions" (
  	...
  	"cart_id" integer,
  	"amount" numeric,
  	"currency" "enum_transactions_currency" DEFAULT 'COP',
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  ALTER TABLE "carts_attributes" ADD CONSTRAINT "carts_attributes_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."carts"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE INDEX "carts_attributes_order_idx" ON "carts_attributes" USING btree ("_order");
  CREATE INDEX "carts_attributes_parent_id_idx" ON "carts_attributes" USING btree ("_parent_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "carts_attributes" CASCADE;
